...
class InputGuardrail:
    ...
    def validate_otp(v):
        c = v.strip()
        if OTP_REGEX.match(c):
            return c, None
        return None, "Please enter a valid OTP (4-6 digits)."

    @staticmethod
    def validate_pan(v):
        c = v.strip().upper()
        if PAN_REGEX.match(c):
            return c, None
        return None, "Invalid PAN. Should be 10 chars: 5 letters, 4 digits, 1 letter (e.g., ABCDE1234F)."

    @staticmethod
    def validate_gstn(v):
        c = v.strip().upper()
        if GSTN_REGEX.match(c):
            return c, None
        return None, "Invalid GSTN. Enter a valid 15-character GSTN (e.g., 07AADCS8891H1ZU)."

    @staticmethod
    def validate_cin(v):
        c = v.strip().upper()
        if CIN_REGEX.match(c):
            return c, None
        return None, "Invalid CIN. Should be 21 chars (e.g., U74300WB1987PTC041861)."

    @staticmethod
    def validate_din(v):
        c = v.strip()
        if DIN_REGEX.match(c):
            return c, None
        return None, "Invalid DIN. Should be exactly 8 digits."
    ...
    def validate_udyam(v):
        ...
        if UDYAM_REGEX.match(c):
            return c, None
        return None, "Invalid Udyam format. Expected: UDYAM-XX-00-0000000. Enter 'No' if not applicable."

    @staticmethod
    def parse_amount_lakhs(v):
        cr = re.search(r'([\d.]+)\s*(crore|cr)', v.lower())
        if cr:
            return float(cr.group(1)) * 100, None
        lk = re.search(r'([\d.]+)\s*(lakh|lac|l\b)', v.lower())
        if lk:
            return float(lk.group(1)), None
        try:
    ...
    def check_loan_limits(lakhs):
        ...
        if lakhs > MAX_LOAN_LAKH:
            return {
                "type": "block",
                "message": f"Maximum unsecured loan is Rs.{MAX_LOAN_LAKH} Lakh (Rs.2 Crore). "
                           f"For higher amounts, contact our secured lending team.",
            }
        return None
...
class OutputGuardrail:
    ...
    BLOCKED = [
        "guaranteed", "100% approval", "definitely approved",
        "your credit score", "your bureau score", "internal risk score", "your cibil",
    ]
        ...
        if node == "NODE_ONBOARD":
            if data_extracted.get("mobile"):
                triggers.append({
                    ...
                    "action": "Scanning blacklist and mobile vintage.",
                    "status": "Trigger OTP",
                })
            if data_extracted.get("otpVerified"):
                triggers.append({
                    "agent": "OTP_VAL",
                    "action": "Authenticating session token.",
                    "status": "Verified",
                })
                triggers.append({
                    "agent": "BUREAU_AGENT",
                    "action": (
                        "Triggered CIC soft-pull\n"
                        "> Task: Verify NTC\n"
            ...
            if data_extracted.get("isGSTRegistered") is not None:
                ...
                triggers.append({
                    "agent": "ROUTING_LOGIC",
                    "action": f"Directing to workflow={path}.",
                    "status": "ROUTE",
                })
            ...
            if data_extracted.get("legalNameConfirmed"):
                triggers.append({
                    ...
                    "action": "Cross-referencing trade name with GST database.",
                    "status": "Verified",
                })
            if data_extracted.get("cin"):
                triggers.append({
                    "agent": "MCA_ORCHESTRATOR",
                    "action": "Syncing with MCA Master Data for director names and status.",
                    "status": "MCA Sync Complete",
                })

        elif node == "NODE_GST_APPLICANT":
            if data_extracted.get("applicantName"):
                triggers.append({
                    "agent": "FUZZY_MATCH_AGENT",
                    "action": "Comparing name against Director Master list from MCA.",
                    "status": "Identity Match",
                })
            if data_extracted.get("din_or_role"):
                triggers.append({
                    "agent": "DIRECTOR_VERIFICATION_AGENT",
                    "action": "Validating DIN state and eligibility.",
            ...
            if data_extracted.get("pan"):
                triggers.append({
                    "agent": "PAN_VAL_AGENT",
                    "action": "Validating PAN status and ownership.",
                    "status": "PAN Verified",
                })
            if data_extracted.get("udyam"):
                ...
                triggers.append({
                    "agent": "UDYAM_AGENT",
                    "action": f"Udyam lookup: {data_extracted['udyam']}",
                    "status": st,
                })
            if data_extracted.get("industry"):
                triggers.append({
                    "agent": "SECTOR_AGENT",
                    "action": "Benchmarking industry-specific risk parameters.",
                    "status": "Sector Mapped",
                })
            if data_extracted.get("applicantName"):
                triggers.append({
                    "agent": "IDENTITY_VERIFICATION_AGENT",
                    "action": (
                        "Validating applicant name against PAN records.\n"
                        "> Cross-referencing with entity ownership data"
                    ),
                    "status": "Applicant Logged",
                })
            if data_extracted.get("designation"):
                triggers.append({
                    "agent": "SIGNATORY_AUTH_AGENT",
                    "action": (
                        f"Recording applicant role: {data_extracted['designation']}.\n"
                        "> Flagging for authorised signatory verification at disbursal"
                    ),
                    "status": "Designation Verified",
                })
            ...
            if data_extracted.get("revenue"):
                triggers.append({
                    "agent": "OFFER_INPUT",
                    "action": "Extracting numerical features for financial benchmarking.",
            ...
            if data_extracted.get("loanAmount"):
                triggers.append({
                    "agent": "BUREAU_AGENT",
                    "action": (
                        "Trigger obligation impact analysis.\n"
                        "> BRE_Agent: Eligibility computation initiated"
                    ),
                    "status": "Eligibility Computing",
                })
            if data_extracted.get("loanPurpose"):
                triggers.append({
                    "agent": "CLASSIFICATION_AGENT",
                    "action": "Mapping loan purpose to product sub-category.",
                    "status": "Purpose Classified",
                })

        elif node == "NODE_SUMMARY":
            triggers.append({
                "agent": "SUMMARY_VALIDATION_AGENT",
                "action": "Finalizing profile metadata and data integrity check.",
                "status": "Summary Confirmed",
            })

        elif node == "NODE_GST_PRELIM_OFFER":
            triggers.append({
                "agent": "OFFER_STRATEGY_ENGINE",
                "action": "Preliminary offer generated. Virtual RM suggesting enhanced limit.",
                "status": "Prelim Offer",
            })
            if data_extracted.get("wantsGSTConsent"):
                triggers.append({
                    "agent": "OFFER_STRATEGY_ENGINE",
                    "action": "User opted for enhanced limit. Triggering upsell flow.",
            ...
            if data_extracted.get("gstConsentStarted"):
                triggers.append({
                    "agent": "GST_CONSENT_ORCHESTRATOR",
                    "action": "Requesting digital consent token.",
                    "status": "Consent Pending",
                })
            if data_extracted.get("selectedGSTNs"):
                triggers.append({
                    "agent": "GSTN_SELECTION_AGENT",
                    "action": "Mapping nodes for multi-GST aggregation.",
                    "status": "Consent Mapping",
                })
            if data_extracted.get("gstUsername"):
                triggers.append({
                    "agent": "AUTH_ORCHESTRATOR",
                    "action": "Initializing session with GST portal.",
                    "status": "Auth Started",
                })
            if data_extracted.get("gstOtpVerified"):
                triggers.append({
                    "agent": "OTP_VALIDATION_AGENT",
                    "action": "Verifying second factor for portal access.",
                    "status": "GSTN Verified",
                })

        elif node == "NODE_DOCUMENTS":
            if data_extracted.get("bankStatementUploaded"):
                triggers.append({
                    "agent": "BSA_AGENT, TRIANGULATION_MODEL",
                    "action": (
                        "Extracting text & metadata.\n"
                        "> Task: Transaction pattern analysis\n"
                        "> Task: BSA health score and EWS alerts"
                    ),
                    "status": "BSA Processing",
                })
            if data_extracted.get("financialsUploaded"):
                triggers.append({
                    "agent": "FINANCIAL_AGENT, PEER COMPARISON",
                    "action": (
                        "Parsing P&L and Balance Sheet.\n"
                        "> Compute financial features\n"
                        "> Flag inconsistencies via forensic model"
                    ),
                    "status": "Financials Analyzed",
                })

        elif node == "NODE_OFFER":
            triggers.append({
                "agent": "CREDIT_POLICY_ENGINE",
                "action": (
                    "Offer finalization.\n"
                    "> Composite Risk Score\n"
                    "> Model Confidence check\n"
                    "> LTV Compliance: Checked"
                ),
                "status": "Offer Finalized",
            })

        elif node == "NODE_CLOSURE":
            triggers.append({
                "agent": "WORKFLOW_EXIT",
                "action": "Dispatching lead to Salesforce.\n> RM notified via Slack",
                "status": "Lead Exported",
            })
...
class NodeRouter:
    ...
    def determine_node(ud, msg_count):
        ...
        # ── Phase 0: Onboard ──
        if (not ud.get("mobile")
                or not ud.get("otpVerified")
...
# ══════════════════════════════════════════════════════════════════════════════
#  INPUT GUARDRAIL RUNNER — Pre-LLM checks
# ══════════════════════════════════════════════════════════════════════════════

def _guardrail_response(message, log, status, node, guardrail=None, input_type="text"):
    """Helper to build a guardrail bypass response."""
    return {
        "message": message,
        "logEntry": f"GUARDRAIL: {log}",
        "logStatus": status,
        "inputType": input_type,
        "dataExtracted": {},
        "guardrailFlag": guardrail,
        "isSummary": False,
        "summaryData": None,
        "currentNode": node,
    }
...
def run_input_guardrails(msg, user_data, node):
    """Validate user input before sending to LLM. Returns (msg, guardrail_response|None)."""
    msg = msg.strip()
    g = InputGuardrail()
    ...
    for pat in injection_patterns:
        if re.search(pat, msg.lower()):
            return msg, _guardrail_response(
                "I'm AIWA, your loan assistant. How can I help with your business loan?",
                "Injection blocked", "BLOCKED", node,
                guardrail={"type": "warn", "message": "Unauthorized input."},
            )
    ...
    if ex == "mobile":
        ...
        if err:
            return msg, _guardrail_response(err, "Invalid mobile", "FAIL", node)

    elif ex == "otp":
        _, err = g.validate_otp(msg)
        if err:
            return msg, _guardrail_response(err, "Invalid OTP", "FAIL", node)

    elif ex == "gstn":
        _, err = g.validate_gstn(msg)
        if err:
            return msg, _guardrail_response(err, "Invalid GSTN", "FAIL", node)

    elif ex == "cin":
        skip_keywords = ['no', 'na', 'not applicable', 'none', 'skip', 'not mca registered', 'not mca']
        if msg.lower() not in skip_keywords:
            _, err = g.validate_cin(msg)
            if err:
                return msg, _guardrail_response(err, "Invalid CIN", "FAIL", node)

    elif ex == "din":
        if msg.strip().replace(' ', '').isdigit():
            _, err = g.validate_din(msg.strip())
            if err:
                return msg, _guardrail_response(err, "Invalid DIN", "FAIL", node)

    elif ex == "pan":
        _, err = g.validate_pan(msg)
        if err:
            return msg, _guardrail_response(err, "Invalid PAN", "FAIL", node)

    elif ex == "udyam":
        _, err = g.validate_udyam(msg)
        if err:
            return msg, _guardrail_response(err, "Invalid Udyam", "FAIL", node)

    elif ex == "loanAmount":
        amt, err = g.parse_amount_lakhs(msg)
        if err:
            return msg, _guardrail_response(err, "Bad amount", "FAIL", node)
        flag = g.check_loan_limits(amt)
        if flag:
            return msg, _guardrail_response(
                flag["message"], f"Loan {amt}L out of range", "BLOCKED", node, guardrail=flag,
            )
        ...
        if flag:
            return msg, _guardrail_response(
                flag["message"], f"Blacklisted: {msg}", "BLOCKED", "NODE_CLOSURE",
                guardrail=flag, input_type="end",
            )
...
# ══════════════════════════════════════════════════════════════════════════════
#  PROMPT BUILDER & HISTORY TRIMMER
# ══════════════════════════════════════════════════════════════════════════════
...
def _error_response(message, log_entry, log_status):
    """Build a safe error response that won't break the frontend."""
    return {
        "message": message,
        "logEntry": log_entry,
...
def safe_process(current_node, parsed, user_data, persona_logs):
    ...
    try:
        ...
        return jsonify(_error_response(
            "Processing issue. Please repeat your response.",
            f"PARSE_ERROR: {e}", "ERROR",
        )), 200
...
def chat():
    try:
        ...
        if resp.status_code == 401:
            return jsonify({"error": "Invalid API key. Set ANTHROPIC_API_KEY in .env or underwrite.py"}), 401

        if resp.status_code == 429:
            return jsonify({
                "message": "I'm processing. Please wait a moment and try again.",
                "logEntry": "RATE_LIMIT",
                "logStatus": "RATE_LIMIT",
                "inputType": "text",
                "dataExtracted": {},
                "guardrailFlag": None,
                "isSummary": False,
        ...
        try:
            ...
            if m:
                ...
                raise ValueError(f"No JSON: {clean[:200]}")
        ...
        updates, persona_logs = trigger_models(current_node, user_data)
        parsed["dataExtracted"].update(updates)

        simulated = run_persona_agents(current_node, persona_data, user_data)
        parsed["dataExtracted"].update(simulated)

        # ── Process and return ──  [FIX: was two return statements, second unreachable]
